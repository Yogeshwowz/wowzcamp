...
class Dashboard_model extends CI_Model{
    ...
    protected $room_category = 'room_category';
	   ...
	   $where  =" where status=1 ";
		...
		if(!empty($searcharray['adults'])){
				...
				foreach($searcharray['adults'] as $value){
					...
					$where  .=" AND  (((guest='2' AND  number_of_room >='".$searcharray['startField']."') OR (guest='".$sumadults."' AND number_of_room >='1')))";
				
					
				}
		if(!empty($_POST['singlepage'])){
			 $where  .=" AND  hotel_id='".$searcharray['hotel_id']."'";
			
			$Sql ="select ".$this->properties.".*,".$this->properties_type_size.".properties_type_size,".$this->hotels.".hotel_name as hotel_name,".$this->cities.".name as city ,".$this->room_category.".room_category_name     from  ".$this->properties."
				...
				left join  ".$this->room_category." on ".$this->room_category.".room_category_id=".$this->properties.".room_category
				".$where .'  order by price ASC';
				
				
		}else{
			$Sql = "select properties_type_size.properties_type_size,hotels.hotel_name as hotel_name,cities.name as city,l.* 
				from properties  l
				inner join (
				select 
				hotel_id, min(price) as latest 
				from properties ".$where."   
				group by hotel_id
				) r
				on l.price = r.latest and l.hotel_id = r.hotel_id
				left join  ".$this->properties_type_size." on ".$this->properties_type_size.".id=l.ptype 
				left join  ".$this->hotels." on ".$this->hotels.".id=l.hotel_id 
				left join  ".$this->cities." on ".$this->cities.".id=l.location".
				$where .' GROUP BY hotel_id   order by'.$orderby;
		...
		if(!empty($searcharray['adults'])){
			...
			if(!empty($_POST['singlepage'])){
				...
				left join  ".$this->cities." on ".$this->cities.".id=".$this->properties.".location
				left join  ".$this->room_category." on ".$this->room_category.".room_category_id=".$this->properties.".room_category
				".$where." ORDER BY price ASC LIMIT ".$rowno." , ".$rowperpage;
			}else{
				$Sql = "select properties_type_size.properties_type_size,hotels.hotel_name as hotel_name,cities.name as city,l.* 
				from properties  l
				inner join (
				...
				left join  ".$this->hotels." on ".$this->hotels.".id=l.hotel_id 
				left join  ".$this->cities." on ".$this->cities.".id=l.location".
				$where .' GROUP BY hotel_id   order by'.$orderby." LIMIT ".$rowno." , ".$rowperpage;
			}
			...
			foreach($result as $key=> $allRooms){
					...
					$array[$key]['room_category_name'] =  $allRooms['room_category_name'];
					...
					$array[$key]['ptype'] = $allRooms['ptype'];
					//Property Images
		...
		public function getsingleDetails($id) {
			...
			$array = array();
					...
					$array['hotel_id'] =  $result['hotel_id'];
 ...
 public function bookNow() {
		...
		$update =$this->db->where('browser_id',$browser_id)->update($this->booking,$updateData);

		
		$where  =" where ".$this->booking.".browser_id='".$browser_id."' ";
		
		$this->db->select('room_category')->from($this->properties);
		$this->db->where('id',$pid);  
		$queryR = $this->db->get();
		$rs = $queryR->row_array();
		

		$this->db->select('guestcount')->from($this->booking);
		$this->db->where('browser_id',$browser_id);  
		$queryRB = $this->db->get();
		$rsB = $queryRB->row_array();

		if($rs['room_category']==1 || $rs['room_category']==2){
			if($rsB['guestcount'] >0){
				if(($rsB['guestcount'] % 2) == 0){  
				  	$num1 = $rsB['guestcount'];
					$num2 = 2;
					$roomscount = $num1 / $num2;
				}
			}
			$data = array(
						'roomscount' =>$roomscount,
		...
		if($rs['room_category']==3 || $rs['room_category']==4){
			$data_1 = array(
						'roomscount' =>1,
						
						);
			 $this->db->where('browser_id', $browser_id);
			$this->db->update($this->booking, $data_1);	
