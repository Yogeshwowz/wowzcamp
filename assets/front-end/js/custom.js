...
var locationid=$( "#locationid option:selected" ).text();
 ...
 $(".search-filter").change(function() { 
     ...
     $(this).parent().parent().parent().parent().parent().find('#sfilter-1').removeClass('show');
$(this).parent().parent().parent().parent().parent().find('a.sf-btn').addClass('collapsed');
$(this).parent().parent().parent().parent().parent().find('a.sf-btn').attr('aria-expanded',false);

$(this).parent().parent().parent().parent().parent().find('#sfilter-3').removeClass('show');
$(this).parent().parent().parent().parent().parent().find('a.sf-btn').addClass('collapsed');
$(this).parent().parent().parent().parent().parent().find('a.sf-btn').attr('aria-expanded',false);

$(this).parent().parent().parent().parent().parent().find('#sfilter-4').removeClass('show');
$(this).parent().parent().parent().parent().parent().find('a.sf-btn').addClass('collapsed');
$(this).parent().parent().parent().parent().parent().find('a.sf-btn').attr('aria-expanded',false); 



$(this).parent().parent().parent().parent().parent().find('#sfilter-6').removeClass('show');
$(this).parent().parent().parent().parent().parent().find('a.sf-btn').addClass('collapsed');
$(this).parent().parent().parent().parent().parent().find('a.sf-btn').attr('aria-expanded',false); 
			...
			function createTable(result,sno){
				...
				if(countTable > 0){
					for(index in result){
						...
						var linktype='list';
						var pathurl = baseUrl+'single-details/'+ptype+'/'+linktype+'/'+pid;
               ...
               tr += '</div>\
                <div class="col-lg-5 pt-4 d-flex flex-column">\
					...
					<h5 class="propertynamecls" id="propertynamecls">'+propertyname+'</h5>\
                    ...
                    tr += '<div class="list-des"><p>Description</p>\
                     ...
                     tr += '<div class="listing-footer mt-auto">\
                        <span class="listing-price">\
                            Our Price <span class="cur-price main-color1"><i class="fas fa-rupee-sign"></i>'+after_discount_price+'</span>\
                            <span class="main-price px-3"><i class="fas fa-rupee-sign"></i> '+price+'</span>\
                            <span class="off-price">'+discount+'% off</span>\
                        </span>\
                        <p>'+rating_status+'</p>\
                        <span class="d-flex justify-content-between">\
                            <a href="javascript:void(0);" class="btn main-btn2 my-2 my-sm-0 view-details" data-path="'+pathurl+'" rel="'+id+'">Select Room Now</a>\
                            <a  id="book_now" href="javascript:void(0);" class="btn main-btn my-2 my-sm-0 book_now" rel="'+id+'">BOOK NOW</a>\
                        </span>\
                      </div>\
                </div>\
            </div>\
  ...
  if(d_check_out >= d_check_in){
	...
	// Load pagination
			...
			function createTableForPage(result,sno){
				...
				var roomcount_1 = $("span#roomcount").text();
				var roomcount_2 = $("span#roomcount").text();
				if(roomcount_1!=''){
					var roomcount = $("span#roomcount").text();
				}
				if(roomcount_2!=''){
					var roomcount = $("span#roomcount_mo").text();
				}
				...
				if(countTable > 0){
					for(index in result){
						...
						var property_type = result[index].property_type;
						var hotel_name = result[index].hotel_name;
						var propertyname = result[index].room_category_name;
						var price = result[index].price;
						var after_discount_price = result[index].after_discount_price;
						var city = result[index].city;
						var description = result[index].description;
						...
						var amenities = result[index].amenities.slice(0,3);
						var rating_status = result[index].rating_status;
						var proptype = result[index].ptype;
						...
						var tr='<div class="b-1 br-5 p-3 mb-4">\
                                            ...
                                            <div class="row pt-3">\
                                                ...
                                                if(proptype==1 || proptype==2){
                                                	if(roomcount >1){
                                                		tr +=   '<a href="javascript:void(0);" class="main-btn2 br-5 py-1 px-2 w-50 view-details" data-path="'+pathurl+'" rel="'+id+'">Select '+roomcount+ ' Room Now</a>';
                                                	}else{
                                                		tr +=   '<a href="javascript:void(0);" class="main-btn2 br-5 py-1 px-2 w-50 view-details" data-path="'+pathurl+'" rel="'+id+'">Select Room Now</a>';
                                                	}
                                                	
                                                }
                                                 if(proptype==3 || proptype==4){
                                                	tr +=   '<a href="javascript:void(0);" class="main-btn2 br-5 py-1 px-2 w-50 view-details" data-path="'+pathurl+'" rel="'+id+'">Select  Room Now</a>';
                                                }
                                                    tr +=   ' <a id="book_now" href="javascript:void(0);" class="main-btn br-5 py-1 px-2 w-50 ml-3 book_now" rel="'+id+'">BOOK NOW</a>\
                                                </div>\
                                            </div>\
                                        </div>';
						$('#postsList').append(tr);
    ...
    $(addButton).click(function(){
		...
		var  sum = parseInt(guest_span) + parseInt(2);
		$('#guest_span').text(sum);
    ...
    $(".search-filter-single").change(function() { 
		...
		$( "#search-form-home" ).submit(function( event ){ //on form submit       
		var check_in =$( "#check_in" ).val();
		var check_out=$( "#check_out" ).val();
		var d_check_in = Date.parse(check_in) / 1000;
		var d_check_out = Date.parse(check_out) / 1000;
...
$("#s_about_hotel").click(function() {
    $('html, body').animate({
        scrollTop: $('#des-tab').offset().top -170 
    }, 'slow');
});
$("#s_amenities").click(function() {
    $('html, body').animate({
        scrollTop: $("#amenties-tab").offset().top -170
    }, 'slow');
});
$("#s_view_amenties").click(function() {
    $('html, body').animate({
        scrollTop: $("#amenties-tab").offset().top -170
    }, 'slow');
});
$("#s_choose_room").click(function() {
    $('html, body').animate({
        scrollTop: $("#c_room").offset().top -170
    }, 'slow');
});
